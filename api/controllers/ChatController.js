module.exports = {
	...
	private: function(req, res) {
		...
		User.findOne(req.session.users[socketId].id).exec(function(err, sender) {
			...
			User.publish(req.param('to'), 'privateMessage', {from: sender, msg: req.param('msg')});
	...
	public: function(req, res) {
		...
		User.findOne(req.session.users[socketId].id).exec(function(err, user) {
			...
			Room.publish(req.param('room'), 'publicMessage', {room:{id:req.param('room')}, from: user, msg: req.param('msg')}, req.socket);
